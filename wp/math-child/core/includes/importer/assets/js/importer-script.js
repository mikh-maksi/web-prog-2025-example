var Whizzie = (function($) {
  ...
  function window_loaded() {
    ...
    $('.importer-menu li.step').each(function(index) {
    ...
    $('.importer-menu li .detail').each(function(index) {
      $(this).attr('data-height', $(this).innerHeight());
      $(this).addClass('scale-down');
    });


    $('.importer-menu li.step').css('height', '100%');
    $('.importer-menu li.step:first-child').addClass('active-step');

    $('.importer-wrap').addClass('loaded');
    ...
    $('.do-it').on('click', function(e) {
      ...
      $('.importer-wrap').addClass('spinning');
  ...
  function do_next_step(btn) {
    ...
    current_step.fadeOut(500, function() {
      ...
      $('.importer-wrap').removeClass('spinning');
    });
  }
  ...
  function PluginManager() {
        ...
        if (typeof response.url != 'undefined') {
          ...
          if (response.hash == current_item_hash) {
            ...
            jQuery.post(response.url, response, function(response2) {
              process_current();
              $current_node.find('span').text(response.message + project_management_whizzie_params.verify_text);
            }).fail(ajax_callback);
          }
    ...
    function process_current() {
      if (current_item) {
        ...
        jQuery.post(project_management_whizzie_params.ajaxurl, {
          action: 'setup_plugins',
          wpnonce: project_management_whizzie_params.wpnonce,
          slug: current_item
    ...
    return {
      init: function(btn) {
        $('.mi-wizard-plugins').addClass('installing');
        complete = function() {
          do_next_step();
        };
        find_next();
  ...
  function WidgetManager() {
    function import_widgets() {
      jQuery.post(
        project_management_whizzie_params.ajaxurl, {
  ...
  function ContentManager() {
    ...
    return {
      init: function(btn) {
        $('.mi-setup-pages').addClass('installing');
        $('.mi-setup-pages').find('input').prop("disabled", true);
        complete = function() {
          loading_content();
...
jQuery(document).ready(function() {
  ...
  jQuery('#ti-demo-setup-guid .ti-setup-menu').attr("checked", "checked");

  jQuery('#ti-setup-menu').click(function() {
    jQuery('#ti-demo-setup-guid .ti-setup-menu').css("display", "block");
    jQuery('#ti-demo-setup-guid .ti-setup-contact').css("display", "none");
    jQuery('#ti-demo-setup-guid .ti-setup-widget').css("display", "none");
  });

  jQuery('#ti-setup-contact').click(function() {
    jQuery('#ti-demo-setup-guid .ti-setup-menu').css("display", "none");
    jQuery('#ti-demo-setup-guid .ti-setup-contact').css("display", "block");
    jQuery('#ti-demo-setup-guid .ti-setup-widget').css("display", "none");
  });

  jQuery('#ti-setup-widget').click(function() {
    jQuery('#ti-demo-setup-guid .ti-setup-menu').css("display", "none");
    jQuery('#ti-demo-setup-guid .ti-setup-contact').css("display", "none");
    jQuery('#ti-demo-setup-guid .ti-setup-widget').css("display", "block");
  });
